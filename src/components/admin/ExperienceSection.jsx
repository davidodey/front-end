...
function ExperienceSection() {
    ...
    // Initialize companies with existing profile experiences
    useEffect(() => {
        if (profile?.experiences?.length) {
            profile.experiences.forEach((experience) => {
                dispatch(addCompany(experience));
            });
        }
    }, [profile, dispatch]);
    ...
    const handleSave = async () => {
        try {
            ...
            const payload = {
                userId: profile.sub,
                experiences: companies.map(({ logoFile, ...rest }) => rest), // Exclude non-serializable fields
            };
        ...
        <div className="admin-section">
            ...
            {companies.length === 0 && (
                <button
                    ...
                    className="admin-save-button add-button"
                >
                    Add Company
                </button>
            )}
